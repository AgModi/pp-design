...
public class EmpReceiverB implements IConsumer{
    ...
    public boolean consume(JsonNode node) {
        log.info("In consumer B for message {}", node);
    ...
    private boolean processMessage(JsonNode node) {
        ...
        if (employeeDto != null) {
            log.info("Returting true from consumer B for message {}", node);
            return true;
        }
        log.info("Returting false from consumer B for message {}", node);
        throw new ListnerFailureException("Listsner B failed to process object" + node);
    }
}
